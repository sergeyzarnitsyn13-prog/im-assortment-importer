...
assert.equal(
  /A\+\+\/A\+\+\+/u.test([lagoonFlatPdfEnergyDraft.salesFeatures, lagoonFlatPdfEnergyDraft.keyFeatures, lagoonFlatPdfEnergyDraft.importantSpecs].flat().join(' ')),
  false,
  'LAGOON flat PDF card must not promote A++/A+++ from SEER/SCOP into public features/specs',
);
...
assert.match(defenderBshiRawExtractorDraft.catalogExtract.importantSpecs.join('\n'), /класс энергоэффективности SEER\/SCOP (А|A)\+\+\/(А|A)\+\+\+/u, 'Defender BSHI importantSpecs must include SEER/SCOP energy class from technical row');
...
const assertCatalogPassportSalesFieldsAreEmpty = (draft, label) => {
  assert.equal(draft.shortDescription, '', `${label} shortDescription must be stripped for catalog passport`);
  assert.equal(draft.positioning, '', `${label} positioning must be stripped for catalog passport`);
  assert.deepEqual(draft.targetClient, [], `${label} targetClient must be stripped for catalog passport`);
  assert.equal(draft.mainSalesIdea, '', `${label} mainSalesIdea must be stripped for catalog passport`);
  assert.deepEqual(draft.salesArguments, [], `${label} salesArguments must be stripped for catalog passport`);
  assert.equal(draft.clientSpeech, '', `${label} clientSpeech must be stripped for catalog passport`);
  assert.deepEqual(draft.whenRecommend, [], `${label} whenRecommend must be stripped for catalog passport`);
  assert.deepEqual(draft.whenNotRecommend, [], `${label} whenNotRecommend must be stripped for catalog passport`);
};

const assertCatalogExtractDiagnostics = (draft, label) => {
  assert.ok(draft.catalogExtract, `${label} catalogExtract must be present`);
  assert.match(draft.catalogExtract.extractionQuality, /^(good|partial|needs_review)$/, `${label} extractionQuality must be valid`);
  assert.equal(Array.isArray(draft.catalogExtract.diagnostics.warnings), true, `${label} catalogExtract diagnostics warnings must be an array`);
  assert.equal(Array.isArray(draft.catalogExtract.unparsedTechnicalRows), true, `${label} unparsedTechnicalRows must be an array`);
};

for (const [label, draft] of [
  ['Velure', velureDraft],
  ['Eco Cool', ecoCoolDraft],
  ['Smart Inverter', smartInverterDraft],
  ['Smart Inverter EVO', smartInverterEvoDraft],
  ['Platinum X4', platinumX4Draft],
  ['Discovery BSVI', discoveryBsviRawExtractorDraft],
  ['Lagoon BSDI', lagoonBsdiRawExtractorDraft],
  ['Defender BSHI', defenderBshiRawExtractorDraft],
]) {
  assertCatalogPassportSalesFieldsAreEmpty(draft, label);
  assertCatalogExtractDiagnostics(draft, label);
}
